...
Confronto: salva il report in ``Adriano_graph/data/output/compare_test_results.json`` (override con ``-o``).
...
DEFAULT_MODEL = ADRIANO_GRAPH_ROOT / "data" / "stage_3" / "test" / "extracted_graph_test.json"
DEFAULT_COMPARE_OUTPUT = ADRIANO_GRAPH_ROOT / "data" / "output" / "compare_test_results.json"
    ...
    **compare** — Per ogni ``*.json`` in ``--human-dir`` (saltati i file senza un
    ...
    Opzioni ``compare``:
        ...
        --model PATH
            ...
            ``data/stage_3/test/extracted_graph_test.json``.
        -o, --output PATH
            File del report di confronto. Default:
            ``data/output/compare_test_results.json``.
        --print
