# Inference — GraphRAG preliminare

Test locale: **domanda → chunk (BGE-M3) + grafo risolto (1 hop) → Qwen via LM Studio**.

Tutto vive in questa cartella; legge i dati da `Adriano_graph/data/` senza modificarli.
...
4. **Dati pipeline** (già presenti nel repo):

   - `Adriano_graph/data/stage_2/chunks.json`
   - `Adriano_graph/data/stage_4/3_resolve/resolved_graph.json`
...
| Chiave | Significato |
|--------|-------------|
| `chunks_path` / `graph_path` | Input testo e grafo |
| `embed_model` / `embed_device` | BGE-M3 e `cuda` o `cpu` |
...
### 1. Costruire l'indice vettoriale (una tantum, o dopo cambio chunk)

Embedda i 310 chunk con BGE-M3 e salva in `data/index/`:
...
Rilancia dopo un nuovo `chunks.json`. Non serve rifare l'indice se cambi solo il grafo.

---

### 2. Verificare il retrieval (senza LLM)
...
### 3. Domanda con risposta (LM Studio acceso)
...
### 5. Chat interattiva (consigliato)
...
- `data/index/chunk_texts.json` creato da `build_index.py` — **rilancia build_index** se manca
...
```
inference/
├── config.yaml          # path e parametri
├── build_index.py       # step 1 — embed chunk
├── ask.py               # domanda singola (opz. --stream)
├── chat.py              # chat interattiva + streaming
├── server.py            # API web FastAPI + SSE (chat browser)
├── smoke_test.py        # 5 domande batch
└── rag/
    ├── session.py       # sessione persistente (caricamento una tantum)
    ├── index.py         # indice vettoriale numpy
...
| Problema | Cosa fare |
|----------|-----------|
| `Indice non trovato` | Esegui `python build_index.py` |
...
- Passare a `enriched_graph.json` quando lo stadio 5 è chiuso  
- Neo4j per query Cypher più ricche  
- Valutazione su `dataset_qa`  
- Formalizzare come stadio 6 della pipeline Adriano_graph  
